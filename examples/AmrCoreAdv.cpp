...
void AmrCoreAdv::ErrorEst(int lev, amrex::TagBoxArray &tags, amrex::Real time,
                          int ngrow) {
      ...
      for (int k = lo.z; k <= hi.z; ++k) {
        for (int j = lo.y; j <= hi.y; ++j) {
          for (int i = lo.x; i <= hi.x; ++i) {
            fab2(i, j, k) =
                dt *
                    (fab(i - 1, j, k) - 2 * fab(i, j, k) + fab(i + 1, j, k) +
                     fab(i, j - 1, k) - 2 * fab(i, j, k) + fab(i, j + 1, k)) /
                    (dx[0] * dx[0]) +
                fab(i, j, k);
          }
        }
...
void AmrCoreAdv::FillPatch(int lev, Real time, MultiFab &mf, int icomp,
...

  {
    ...
    amrex::FillPatchTwoLevels(mf, 0.0, cmf, ctime, fmf, ftime, 0, icomp, ncomp,
